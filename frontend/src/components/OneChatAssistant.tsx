...
const OneChatAssistant: React.FC = () => {
  ...
  const loadAgents = async () => {
    try {
      const response = await fetch(`${apiUrl}/api/onechat/agents`);
  ...
  const loadConversations = async () => {
    try {
      const response = await fetch(`${apiUrl}/api/onechat/conversations`);
      const data = await response.json();
  ...
  const loadConversation = async (convId: string) => {
    try {
      const response = await fetch(`${apiUrl}/api/onechat/conversations/${convId}`);
      const data = await response.json();
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (selectedAgent) {
        requestBody.agent_id = selectedAgent;
      }

      const response = await fetch(`${apiUrl}/api/onechat/converse`, {
        method: 'POST',
    ...
    <Paper sx={{ p: 3, height: '600px', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ flex: 1, overflow: 'auto', mb: 2, border: '1px solid #e0e0e0', borderRadius: 1, p: 1 }}>
        {messages.length === 0 ? (
          <Typography color="text.secondary" sx={{ textAlign: 'center', mt: 4 }}>
            Start a conversation with the 1Chat assistant...
          </Typography>
        ) : (
            ...
            <Box key={message.id} sx={{ mb: 2 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                ...
                <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>
                  {message.role === 'user' ? 'You' : '1Chat Assistant'}
                </Typography>
        ...
        {loading && (
          <Box sx={{ display: 'flex', alignItems: 'center', p: 2 }}>
            ...
            <Typography>1Chat is thinking...</Typography>
          </Box>
        )}
      ...
      <Box sx={{ display: 'flex', gap: 1 }}>
        <TextField
          fullWidth
          variant="outlined"
          placeholder="Ask 1Chat anything..."
          value={input}
...
export default OneChatAssistant;
