...
const MCPServerStatus: React.FC = () => {
  const [status, setStatus] = useState<MCPServerStatusData | null>(null);
  ...
  const fetchStatus = async () => {
    ...
    try {
      ...
      setError(err?.message || 'Failed to fetch MCP Server status');
      setStatus(null);
    } finally {
      setLoading(false);
    }
      ...
      <Card sx={{ mb: 2 }}>
        <CardContent>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Typography>Checking MCP Server status...</Typography>
          </Box>
        </CardContent>
      </Card>
    );
